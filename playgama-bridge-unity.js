...
initializeBridge(); // Call initialize directly for adapter

// document.head.appendChild(bridgeScript) // Disabled for GamePix adapter
                ...
                createUnityInstance(
                    CANVAS,
                    {
                        ...
                        showBanner: function(msg, type) {
                            if (window._unityShowError) window._unityShowError(msg);
                            if (type === 'error') console.error(msg);
                            else console.warn(msg);
                        },
                    },
                    function(progress) {
                        onUnityLoadingProgressChanged(progress);
                        if (window._unityProgressUpdate) window._unityProgressUpdate(progress * 100);
                    })
                    .then((unityInstance) => {
                        window.unityInstance = unityInstance
                        if (window._unityHideLoading) window._unityHideLoading();
                        CANVAS.focus()
                        flushMessageQueue()
                    })
                    .catch((error) => {
                        console.error(error)
                        if (window._unityShowError) window._unityShowError(String(error));
                    })
            }
            unityLoader.onerror = () => {
                if (window._unityShowError) window._unityShowError('Failed to load game loader script.');
            }
